(function(){
var U="https://hcdrqycgdcmwnqgahvqc.supabase.co";
var K="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImhjZHJxeWNnZGNtd25xZ2FodnFjIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIyNTc0NzMsImV4cCI6MjA4NzgzMzQ3M30._a88kW2GGWq4KFvcEnkWiu-Eo87wPBK2Y_im77gf0SE";
function sbQ(p,o){
  var h={"apikey":K,"Authorization":"Bearer "+K,"Content-Type":"application/json","Prefer":"return=representation"};
  if(o&&o.headers)Object.keys(o.headers).forEach(function(k){h[k]=o.headers[k];});
  return fetch(U+"/rest/v1/"+p,Object.assign({},o,{headers:h})).then(function(r){return r.json();});
}
function monday(){
...
document.addEventListener("DOMContentLoaded",function(){
  var w=document.getElementById("rpt-week");if(w)w.value=monday();
  issueLoad();
});
var RC=[];
window.rptLoad=function(){
  ...
  Promise.all([
    ...
    RC=Array.isArray(res[0])?res[0]:[];
    var sm={};if(Array.isArray(res[1]))res[1].forEach(function(s){sm[s.item_id]=s;});
    var wrap=document.getElementById("rpt-items-wrap");
    if(!RC.length){wrap.innerHTML="<p style=color:#888;padding:20px>항목없음</p>";return;}
    wrap.innerHTML=RC.map(function(it){
      ...
      var inp;
      if(isH){
        var s100=s.score===100?" selected":"",s50=s.score===50?" selected":"",s0=(s.score===0&&s.score!==undefined)?" selected":"";
        inp="<select data-id="+it.id+" class=rv style=padding:5px;border-radius:5px;border:1px_solid_#ccc;font-size:12px>"
           +"<option value=100"+s100+">완료(100)</option>"
           +"<option value=50"+s50+">부분완료(50)</option>"
           +"<option value=0"+s0+">미완료(0)</option></select>";
      }else{
        var cv=isN?(s.raw_value||""):(s.score!==undefined?s.score:0);
        inp="<input type=number data-id="+it.id+" class=rv min=0 max="+(isN?9999:100)+" value="+cv+" placeholder="+(isN?"실제값":"0-100점")+" style=width:80px;padding:5px;border-radius:5px;border:1px_solid_#ccc;font-size:13px>";
      }
      var mv=s.memo||"";
      return "<div style=background:var(--bg2,#f5f5f3);border-radius:8px;padding:12px;margin-bottom:6px>"
        +"<div style=font-size:13px;font-weight:500;margin-bottom:4px>"+it.title+"</div>"
        +"<div style=font-size:11px;color:#888;margin-bottom:8px>"+TL[it.type]+ti+"</div>"
        +"<div style=display:flex;align-items:center;gap:8px;flex-wrap:wrap>"+inp
        +"<input type=text data-id="+it.id+" class=rm placeholder=메모(선택) value="+mv+" style=flex:1;min-width:100px;padding:5px;border-radius:5px;border:1px_solid_#ccc;font-size:12px></div></div>";
    }).join("");
...
window.rptSave=function(){
  ...
  var rows=RC.map(function(it){
    var ve=document.querySelector(".rv[data-id='"+it.id+"']");
    var me=document.querySelector(".rm[data-id='"+it.id+"']");
  ...
  fetch(U+"/rest/v1/report_weekly_scores",{method:"POST",
    headers:{"apikey":K,"Authorization":"Bearer "+K,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates,return=representation"},
    body:JSON.stringify(rows)
  }).then(function(r){return r.json();}).then(function(d){
    var m=document.getElementById("rpt-msg");
    m.textContent=Array.isArray(d)?"저장완료("+d.length+"개)":"오류:"+JSON.stringify(d).substring(0,50);
    m.style.color=Array.isArray(d)?"#1D9E75":"#c00";
...
window.rptSaveComment=function(){
  ...
  if(!week)return;
  fetch(U+"/rest/v1/report_weekly_reports",{method:"POST",
    headers:{"apikey":K,"Authorization":"Bearer "+K,"Content-Type":"application/json","Prefer":"resolution=merge-duplicates,return=minimal"},
    body:JSON.stringify({week_start:week,sin_comment:cmt})
  }).then(function(){
    var m=document.getElementById("rpt-msg");
    m.textContent="총평저장완료";m.style.color="#1D9E75";
    setTimeout(function(){m.textContent="";},3000);
  });
};
var CI=null;
...
window.issueLoad=function(){
  ...
  if(pEl&&pEl.value)q+="&part=eq."+pEl.value;
  if(sEl&&sEl.value)q+="&status=eq."+sEl.value;
  sbQ(q).then(function(issues){
    ...
    list.innerHTML=issues.map(function(iss){
      ...
      var id2=iss.id;
      return "<div style=background:var(--bg2,#f5f5f3);border-radius:8px;padding:12px;border-left:4px_solid_"+(BC[iss.priority]||"#ccc")+";margin-bottom:4px>"
        +"<div style=display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap>"
        +"<span style=font-size:13px;font-weight:500;flex:1>"+iss.title+"</span>"
        +"<span style=font-size:11px;padding:2px_8px;border-radius:10px;"+(SS[iss.status]||"")+">"+SL[iss.status]+"</span>"
        +"<span style=font-size:11px;color:#888>"+(PL[iss.part]||iss.part)+"</span></div>"
        ...
        +"<button onclick=issueUpdateStatus('"+id2+"','"+NS[iss.status]+"') style=padding:3px_9px;border-radius:5px;background:none;border:1px_solid_#ccc;font-size:11px;cursor:pointer>→"+NL[iss.status]+"</button>"
        +"<button onclick=issueOpenComments('"+id2+"','"+st+"') style=padding:3px_9px;border-radius:5px;background:none;border:1px_solid_#ccc;font-size:11px;cursor:pointer>의견</button>"
        +"</div></div>";
    }).join("");
  });
};
...
window.issueUpdateStatus=function(id,status){
  ...
  if(status==="resolved")data.resolved_at=new Date().toISOString();else data.resolved_at=null;
  fetch(U+"/rest/v1/report_issues?id=eq."+id,{method:"PATCH",
    headers:{"apikey":K,"Authorization":"Bearer "+K,"Content-Type":"application/json","Prefer":"return=minimal"},
...
window.issueOpenComments=function(issueId,title){
  ...
  document.getElementById("iss-comment-modal").style.display="flex";
  sbQ("report_issue_comments?issue_id=eq."+issueId+"&order=created_at.asc").then(function(cs){
    ...
    if(!w)return;
    if(!Array.isArray(cs)||!cs.length){w.innerHTML="<p style=color:#888;font-size:13px>의견없음</p>";return;}
    w.innerHTML=cs.map(function(c){
      var dt=new Date(c.created_at).toLocaleString("ko-KR",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"});
      return "<div style=padding:8px;background:var(--bg2,#f5f5f3);border-radius:6px;margin-bottom:4px>"
        +"<div style=font-size:11px;color:#888;margin-bottom:3px>"+c.author+" · "+dt+"</div>"
...
window.issueCommentSubmit=function(){
  ...
  fetch(U+"/rest/v1/report_issue_comments",{method:"POST",
    headers:{"apikey":K,"Authorization":"Bearer "+K,"Content-Type":"application/json","Prefer":"return=minimal"},
    body:JSON.stringify({issue_id:CI,author:author,content:content})
  }).then(function(){
